    ...
    <div className="min-h-screen" style={bgStyle}>
      <div className="max-w-3xl mx-auto px-4 py-10">
        <header className="text-center space-y-2 mb-8">
          {cfg.logo_url && (
            <img src={cfg.logo_url} alt="Logo" className="mx-auto w-20 h-20 rounded-full ring-8"
        ...
        <div className="grid gap-6">
            ...
            <form onSubmit={onSubmit} className="space-y-3">
              <div>
                ...
                <input type="email" value={email} onChange={e=>setEmail(e.target.value)}
                  className="w-full border rounded-lg px-3 py-2" required placeholder="seu@email.com" />
              </div>
              <div>
                <label className="text-sm">Senha</label>
                <input type="password" value={pass} onChange={e=>setPass(e.target.value)}
                  className="w-full border rounded-lg px-3 py-2" required placeholder="••••••••" />
              </div>
          ...
          {cfg.footer_note && <p className="text-center text-xs text-muted-foreground">{cfg.footer_note}</p>}
        </div>
      </div>
    </div>
  )
